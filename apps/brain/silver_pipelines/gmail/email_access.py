...
class EmailAccess:
    def __init__(self):
        self.service = get_service(self)
        self.labels = list_labels(self)
        self.recent_emails = find_recent_emails(self)
    # ---- Auth / Service ----
    ...
    # ---- Helpers ----
    async def _decode_header_value(self, val: str | None) -> str:
        if not val:
            return ""
    ...
    async def extract_plain_text(self, msg) -> str:
        """
    ...
    async def send_email(self, to_addr: str, subject: str, body_text: str):
    ...
    async def find_recent_emails(self, query: str = "newer_than:7d", max_results: int = 3):
        """
        ...
        for m in messages:
            raw_resp = self.service.users().messages().get(
