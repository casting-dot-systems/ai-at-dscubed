...
from org_tools.fact_checking.functions import (
    create_fact,
    send_to_judge,
...
class FactProcessingEngineCommand(Command):
    prompt: str = Field(default="", description="The prompt to execute")


class FactProcessingEngineStatusEvent(Event):
    status: str = Field(default="", description="The status of the engine")


class FactProcessingEngineToolResultEvent(Event):
    tool_name: str = Field(default="", description="The name of the tool that was called")
    result: str = Field(default="", description="The result of the tool call")
...
class FactProcessingEngine(Engine):
    ...
    async def initialize(self):
        ...
        await self.tool_manager.register_tool(create_fact)
        await self.tool_manager.register_tool(send_to_judge)
        await self.tool_manager.register_tool(deletion_confirmation)
        self.message_bus.register_command_handler(FactProcessingEngineCommand, self.handle_command, self.session_id)
    ...
    async def handle_command(
        ...
        """
        assert isinstance(command, FactProcessingEngineCommand)
        try:
...
async def use_fact_processing_engine(
    ...
    engine = FactProcessingEngine(SessionID(session_id))
